---
name: release
description: Run the symphony-pi release flow end-to-end. Use when the user asks to release patch, minor, major, beta, or an explicit semver; includes non-interactive release execution, GitHub Actions monitoring with gh, draft release note cleanup, and publishing the GitHub Release.
---
...
The script runs audit/build/bundled-license/gatecheck/smoke-pack checks, updates `package.json`, creates a signed release commit, creates a signed tag, and pushes commits and tags.
...
Rewrite the notes according to `docs/release-note-guideline.md`. The generated draft is based on commit logs, so categories and wording may be wrong from the symphony-pi product perspective. Remove internal-only updates, move entries to the correct category, merge intermediate same-release fixes into their related feature, and rewrite commit-message phrasing into release-note prose.

For a second-pass review before publishing, delegate a focused review to another agent:

```bash
RELEASE_URL="https://github.com/d-kimuson/symphony-pi/releases/tag/v0.0.0" # replace
...
```bash
TAG="v0.0.0" # replace
NOTES_FILE="/tmp/symphony-pi-$TAG-release-notes.md"
