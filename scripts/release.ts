...
const readPackageJson = (): PackageJson => {
  ...
  if (
    ...
    console.error('x package.json must contain string name and version fields.');
    process.exit(1);
  }
...
const bumpChoices = (version: string): { name: string; value: string }[] => {
  const { major, minor, patch, pre } = parseVersion(version);

  if (pre !== undefined) {
    const preParts = pre.split('.');
    const preTag = preParts[0] ?? 'beta';
    const preNumber = Number(preParts[1] ?? 0);
    const nextPre = `${major}.${minor}.${patch}-${preTag}.${preNumber + 1}`;
...
const resolveVersion = (versionSpec: string, fromVersion: string): VersionResolveResult => {
  ...
  if (versionSpec === 'beta') {
    ...
    const preTag = preParts[0] ?? 'beta';
    const preNumber = Number(preParts[1] ?? 0);
    return { type: 'ok', version: `${major}.${minor}.${patch}-${preTag}.${preNumber + 1}` };
  }
  ...
  const { version } = await inquirer.prompt<{ version: string }>([
    {
      ...
      choices: [...bumpChoices(currentVersion), { name: 'Custom', value: 'custom' }],
    },
  ]);
...
const runReleaseChecks = (): void => {
  ...
  runOrFail('pnpm', ['smoke:pack'], 'Packed package smoke test');
  console.log('\nAll checks passed.\n');
};

const writeVersion = (nextVersion: string): void => {
...
const main = async (): Promise<void> => {
  ...
  const nextVersion =
    cliOptions.version === undefined
      ? await promptVersion(currentVersion)
      : (() => {
          const result = resolveVersion(cliOptions.version, currentVersion);
